...
public class Bracket {// this class manages our structure, storing teams by region and round, and
    ...
    // storing the winning team for each region so we can display them on our main
   ...
   private void initializeBracket() {
        Collections.sort(teams, new Comparator<Team>() {
            ...
            public int compare(Team t1, Team t2) {
                int regionCompare = t1.getRegion().compareTo(t2.getRegion());
                if (regionCompare != 0) {
                    return regionCompare;
                }
                return Integer.compare(t1.getSeed(), t2.getSeed());
            }
        });
    ...
    // method to set the regional champion when a region tournamnet is completed, so
    // we can display it on the main screen
    // stores the winner in the appropriate variable based on the region they are
    // from
    ...
    // new method to get the regional champion for a specific region, so we can
    // display it on the main screen
    // returns null if thatt region hasnt been completed yet so we can say "no
    // champion selected" on the main screen
    ...
    public void resetBracket() {
        ...
        // ensuring the main screen shows all regions as "no champion selected" when
